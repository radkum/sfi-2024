use crate::{
    sha256_utils::Sha256,
    sig_set::{
        bedet_set::BedetSet, signature::SigBedet, HeurSigHeader, SetHeader, SigHeader, SigSet,
    },
    SigSetError,
};
...
impl SigSetDeserializer {
    ...
    pub fn get_set(&self) -> Result<Box<dyn SigSet>, SigSetError> {
        match self.ser_set_header.magic {
            BedetSet::SET_MAGIC_U32 => Ok(Box::new(self.get_bedet_set()?)),
    ...
    pub(crate) fn get_bedet_set(&self) -> Result<BedetSet, SigSetError> {
        ...
        let mut heurset = BedetSet::new_empty();
        for i in 0..elem_count {
            ...
            let sig_bedet: SigBedet = serde_yaml::from_slice(&self.data[curr_offset..end_offset])?;
            log::info!("Properties: {:?}", sig_bedet);

            let imports = sig_bedet
                .attributes
                .iter()
                .map(|s| member_to_hash(sig_bedet.event_type.as_ref(), s.0, s.1))
                .collect();
